...
void moonphase(int py, int pm, int pd)    /////////////////// main function //////////////
{
  int tpy, tpm, tpd;
  float a0, a1, a2;
  long day0;
  
  RTC.fillByYMD(py, pm, pd);
  day0 = RTC.cdn -1;
  RTC.fillByCDN(day0);
  moonage();
  prevNextDays[0] = moon_out*100;
  day0 = RTC.cdn +1;
  RTC.fillByCDN(day0);
  moonage();
  Moon.light = int(moon_out);
  prevNextDays[1] = moon_out*100;
  day0 = RTC.cdn +1;
  RTC.fillByCDN(day0);
  moonage();
  prevNextDays[2] = moon_out*100;
  RTC.fillByYMD(py, pm, pd);

  
  if ((prevNextDays[1] < prevNextDays[0]) && (prevNextDays[1] < prevNextDays[2]))         //new moon
    Moon.phase = 1;
  else if ((prevNextDays[0] < prevNextDays[1]) && (prevNextDays[2]<prevNextDays[1]))       //full moon
    Moon.phase = 5; 
    
  else if (prevNextDays[2] > prevNextDays[0]) {             //first half of moon cycle  
    a0 = 5000 - prevNextDays[0];
    a1 = 5000 - prevNextDays[1];
    if (a1 < 0)
      a1 = -a1;
    a2 = prevNextDays[2] - 5000;
    if ((a1<=a0) && (a1<=a2))                                     //first quarter
       Moon.phase = 3;      
    else if (prevNextDays[1] < 5000)                              //waxing crescent
       Moon.phase = 2;
    else                                                          //waxing gibbous
       Moon.phase = 4;
    }

  else if (prevNextDays[0] > prevNextDays[2]) {               //second half of moon cycl
    a0 = prevNextDays[0] - 5000;
    a1 = 5000 - prevNextDays[1];
    if (a1 < 0)
      a1 = -a1;
    a2 = 5000 - prevNextDays[2];
    if ((a1<=a0) && (a1<=a2))                                      //third quarter
      Moon.phase = 7;
    else if (prevNextDays[1] < 5000)                               //waning crescent
      Moon.phase = 8;
    else                                                            //waning gibbous
      Moon.phase = 6;
    }   
